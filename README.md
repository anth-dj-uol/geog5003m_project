# Chemical Weapon Fallout Simulation
...
This project provides a model for analyzing chemical weapon fallout. The program is based on project requirements suggested in the University of Leeds GEOG5003M independent project instructions. In this model, a bacterial bomb is released on top of a building in some finite environment. Given input parameters for the environment wind direction and particle fall path probabilities, the model will simulate where the individual bomb particles will land within the environment. This model could be used as a rudimentary application for analyzing which areas of an environment should be evacuated and where resources should be allocated to address environment contamination under such a scenario.

Program output is rendered in a Graphical User Interface (GUI), which also includes editable model parameter fields.

![alt text](screenshot.png "Screenshot of a particle density plot")

### Model Details

The simulated scenario is constructed as follows:
...
This basic model makes the following assumptions in its implementation:
...
- `python` - the source code directory
    - `src` - contains source code for the program
        ...
        - `simulation` - the model package directory
            - `__init__.py` - identifies the `simulation` Python package
            - `particleframework.py` - contains classes used to represent particles
            - `logger.py` - contains utility functions for logging program output
            - `model.py` - contains classes that represent the model
            - `view.py` - contains classes for the program GUI
    - `tests` - contains unit test files
        - `particle_test.py` - unit tests for the particle class
...
This project provides a minimal implementation of a model to simulate chemical weapon fallout. Further development for the project can be performed by forking or sending pull requests to this repository. Detailed documentation of the source code is included as docstrings within the Python files.
...
```
import python.src.simulation.particleframework

help(python.src.simulation.particleframework.Particle)
```

The output from these statements will be similar to the following:
```
Help on class Particle in module python.src.simulation.particleframework:

class Particle(builtins.object)
 |  Particle(particle_fall_settings, wind_settings, start_position, building_height)
 |  
